...
# [PROFILING] Add src to sys.path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'src'))

# [FIX] Prevent double splash by mocking setiastro.saspro.__main__
# This prevents src/setiastro/saspro/__init__.py from importing the real __main__.py
# which would run its top-level _init_splash() again.
import types
dummy_main_mod = types.ModuleType("setiastro.saspro.__main__")
dummy_main_mod.main = lambda: None 
sys.modules["setiastro.saspro.__main__"] = dummy_main_mod

# [PROFILING] Globals
TIMINGS = []
LAST_TIME = time.time()
CURRENT_STEP = "Process Initialization"

def record_step(step_name):
    global LAST_TIME, CURRENT_STEP
    now = time.time()
    duration = now - LAST_TIME
    TIMINGS.append((CURRENT_STEP, duration))
    LAST_TIME = now
    CURRENT_STEP = step_name

def print_timings():
    record_step("Profiling Complete")
    print("\n" + "="*80)
    print(f"{'STARTUP PROFILING REPORT':^80}")
    print("="*80)
    print(f"{'STEP':<50} | {'SECONDS':>10} | {'CUMULATIVE':>10}")
    print("-" * 80)
    cum = 0
    for step, dur in TIMINGS:
        if step == "Profiling Complete": continue
        cum += dur
        print(f"{step:<50} | {dur:10.4f} | {cum:10.4f}")
    print("-" * 80)
    print(f"{'TOTAL STARTUP TIME':<50} | {cum:10.4f}")
    print("="*80 + "\n")
...
# [OPTIMIZATION EXPERIMENT] Removed unused reproject import
# try:
...
# [OPTIMIZATION EXPERIMENT] Removed unused OpenCV import
...
def main():
    ...
    # [PROFILING] Monkeypatch MainWindow.__init__ to profile its internals
    unique_init_steps = []
    
    original_init = AstroSuiteProMainWindow.__init__

    def profiled_init(self, *args, **kwargs):
        record_step("  >> MW: Start __init__")
        
        # [PROFILING] Pre-load and monkeypatch DocManager and LayersDock
        try:
            import setiastro.saspro.doc_manager
            OriginalDocManager = setiastro.saspro.doc_manager.DocManager
            
            class ProfiledDocManager(OriginalDocManager):
                def __init__(self, *args, **kwargs):
                    record_step("  >> MW: DocManager.__init__")
                    super().__init__(*args, **kwargs)
                    record_step("  >> MW: DocManager.__init__ done")
            
            setiastro.saspro.doc_manager.DocManager = ProfiledDocManager
        except Exception as e:
            print(f"Failed to patch DocManager: {e}")

        try:
            import setiastro.saspro.layers_dock
            OriginalLayersDock = setiastro.saspro.layers_dock.LayersDock
            
            class ProfiledLayersDock(OriginalLayersDock):
                def __init__(self, *args, **kwargs):
                    record_step("  >> MW: LayersDock.__init__")
                    super().__init__(*args, **kwargs)
                    record_step("  >> MW: LayersDock.__init__ done")
                
                # Profile _refresh_views which is called in init
                def _refresh_views(self):
                    import time as _time
                    record_step("  >> MW: LayersDock._refresh_views start")
                    # Local import measurement
                    t0 = _time.time()
                    from setiastro.saspro.subwindow import ImageSubWindow
                    t1 = _time.time()
                    if t1-t0 > 0.05:
                        print(f"  [LayersDow] import ImageSubWindow took {t1-t0:.3f}s")
                        
                    super()._refresh_views()
                    record_step("  >> MW: LayersDock._refresh_views end")

            setiastro.saspro.layers_dock.LayersDock = ProfiledLayersDock
        except Exception as e:
            print(f"Failed to patch LayersDock: {e}")

        
        methods_to_profile = [
            "_init_explorer_dock",
            "_init_console_dock",
            "_init_header_viewer_dock",
            "_init_layers_dock",
            "_init_resource_monitor_overlay",
            "_init_status_log_dock",
            "_init_log_dock",
            "_create_actions",
            "_init_menubar",
            "_init_toolbar",
            "_install_command_search",
            "_restore_window_placement",
            "_populate_view_panels_menu",
        ]
        
        for method_name in methods_to_profile:
            if hasattr(self, method_name):
                orig_method = getattr(self, method_name)
                # Capture method_name in closure
                def make_wrapper(mn, om):
                    def wrapped(*margs, **mkwargs):
                        record_step(f"  >> MW: {mn}")
                        return om(*margs, **mkwargs)
                    return wrapped
                setattr(self, method_name, make_wrapper(method_name, orig_method))
        
        result = original_init(self, *args, **kwargs)
        record_step("  >> MW: End __init__")
        return result

    AstroSuiteProMainWindow.__init__ = profiled_init

    # [PROFILING] Wrap setMessage to capture main window loading steps
    if _splash:
        record_step("Starting main()")
        orig_set_message = _splash.setMessage
        def wrapped_set_message(m):
            record_step(m)
            orig_set_message(m)
        _splash.setMessage = wrapped_set_message
