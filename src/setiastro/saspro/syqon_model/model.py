...
def _build_StarXEmulator():
    ...
    class _PReLU_DML(nn.Module):
        """DML-safe PReLU: per-channel weights broadcast as (1,C,1,1), never rank>5."""
    ...
    class GatedConv2d(nn.Module):
        def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
            ...
            self.prelu = _PReLU_DML(num_parameters=out_channels)
    ...
    class StarXEmulator(nn.Module):
        def __init__(self, in_channels=3, out_channels=3):
            ...
            # --- ENCODER ---
            self.e1_1 = nn.Sequential(nn.Conv2d(in_channels, 16, kernel_size=3, padding=1), _PReLU_DML(16))
            self.e1_2 = nn.Sequential(nn.Conv2d(in_channels + 16, 16, kernel_size=3, padding=1), _PReLU_DML(16))
            self.e1_3 = nn.Sequential(nn.Conv2d(in_channels + 16 + 16, 16, kernel_size=3, padding=1), _PReLU_DML(16))
            self.e1_4 = GatedConv2d(in_channels + 16 + 16 + 16, 16, kernel_size=3, padding=1)

            self.e2_1 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, stride=2, padding=1), _PReLU_DML(32))
            self.e2_2 = nn.Sequential(nn.Conv2d(32, 32, kernel_size=3, padding=1), _PReLU_DML(32))
            self.e2_3 = GatedConv2d(64, 32, kernel_size=3, padding=1)

            self.e4_1 = nn.Sequential(nn.Conv2d(96, 64, kernel_size=3, stride=2, padding=1), _PReLU_DML(64))
            ...
            self.e32  = nn.Sequential(nn.Conv2d(192, 256, kernel_size=3, stride=2, padding=1), _PReLU_DML(256))
            self.e64  = nn.Sequential(nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=1), _PReLU_DML(512))
            self.e128 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=2, padding=1), _PReLU_DML(512))
            self.e256 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=2, padding=1), _PReLU_DML(512))

            # --- BOTTLENECK ---
            self.conv_0 = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1), _PReLU_DML(256))

            # --- DECODER ---
            self.conv_1  = nn.Sequential(nn.Conv2d(768, 256, kernel_size=3, padding=1), _PReLU_DML(256))
            self.conv_2  = nn.Sequential(nn.Conv2d(768, 256, kernel_size=3, padding=1), _PReLU_DML(256))
            self.conv_3  = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1), _PReLU_DML(256))
            self.conv_4  = nn.Sequential(nn.Conv2d(448, 256, kernel_size=3, padding=1), _PReLU_DML(256))
            self.conv_d4 = nn.Sequential(nn.Conv2d(384, 128, kernel_size=3, padding=1), _PReLU_DML(128))

            self.d4_shuffle      = GatedConv2d(128, 512, kernel_size=3, padding=1)
            self.pixel_shuffle_d4 = nn.PixelShuffle(2)

            self.conv_d2         = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, padding=1), _PReLU_DML(128))
            self.d2_shuffle      = GatedConv2d(128, 512, kernel_size=3, padding=1)
            self.pixel_shuffle_d2 = nn.PixelShuffle(2)

            self.conv_d1         = nn.Sequential(nn.Conv2d(224, 64, kernel_size=3, padding=1), _PReLU_DML(64))
        ...
        def forward(self, x):
            ...
            d4_pixel_shuffle  = self.pixel_shuffle_d4(self.d4_shuffle(d8_out))
            d2_out            = self.conv_d2(torch.cat([d4_pixel_shuffle, e4_concat], dim=1))
            d2_pixel_shuffle  = self.pixel_shuffle_d2(self.d2_shuffle(d2_out))
            d1_out            = self.conv_d1(torch.cat([d2_pixel_shuffle, e2_concat], dim=1))
            d1_pixel_shuffle  = self.pixel_shuffle_d1(self.d1_shuffle(d1_out))
