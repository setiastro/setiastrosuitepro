...
class StatisticalStretchDialog(QDialog):
    ...
    Auto-refresh pipeline (when enabled):
      • Any control change → _fast_timer (150 ms) → downsampled preview
      • Any control change → _full_timer  (800 ms) → full-res preview
    """

    # Maximum longest-side dimension used for the fast-preview downsample
    _FAST_PREVIEW_MAX_DIM = 512
    ...
    def __init__(self, parent, document: ImageDocument):
        ...
        # ── Target median: slider + spinbox side by side ──────────────────
        self.target_row = QWidget()
        tr_lay = QHBoxLayout(self.target_row)
        tr_lay.setContentsMargins(0, 0, 0, 0)
        tr_lay.setSpacing(6)

        self.sld_target = QSlider(Qt.Orientation.Horizontal)
        self.sld_target.setRange(1, 990)        # 0.001 .. 0.990
        self.sld_target.setValue(250)           # 0.25 default
        self.sld_target.setToolTip(self.tr("Target background median (0.001–0.990)"))
        tr_lay.addWidget(self.sld_target, 1)
        ...
        self.spin_target.setFixedWidth(72)
        tr_lay.addWidget(self.spin_target)

        # ── Auto-refresh toggle ───────────────────────────────────────────
        self.chk_auto_refresh = QCheckBox(self.tr("Auto-refresh preview"))
        self.chk_auto_refresh.setChecked(False)
        self.chk_auto_refresh.setToolTip(self.tr(
            "When enabled, any control change automatically updates the preview.\n"
            "A fast downsampled preview fires quickly; full-res follows after a short settle."
        ))
        ...
        self.sld_bp.setRange(50, 1000)
        self.sld_bp.setValue(500)
        ...
        self.sld_luma_blend.setRange(0, 100)
        self.sld_luma_blend.setValue(60)
        ...
        # ──────────────────────────────────────────────────────────────────
        form = QFormLayout()
        form.addRow(self.tr("Target median:"), self.target_row)
        form.addRow("", self.chk_auto_refresh)
        ...
        main_lay.addLayout(left, 0)
        main_lay.addLayout(right, 1)

        # ──────────────────────────────────────────────────────────────────
        # Wiring
        # ──────────────────────────────────────────────────────────────────

        # Target median: slider ↔ spinbox sync
        def _on_target_slider(v: int):
            val = v / 1000.0
            self.spin_target.blockSignals(True)
            self.spin_target.setValue(val)
            self.spin_target.blockSignals(False)
            self._on_any_control_changed()
            self._suggest_hdr_knee_from_target()

        def _on_target_spin(val: float):
            self.sld_target.blockSignals(True)
            self.sld_target.setValue(int(round(val * 1000)))
            self.sld_target.blockSignals(False)
            self._on_any_control_changed()
            self._suggest_hdr_knee_from_target()

        self.sld_target.valueChanged.connect(_on_target_slider)
        self.spin_target.valueChanged.connect(_on_target_spin)
        ...
        self.chk_curves.toggled.connect(lambda _: self._on_any_control_changed())
        self.sld_curves.valueChanged.connect(lambda v: (
            self.lbl_curves_val.setText(f"{v/100:.2f}"),
            self._on_any_control_changed(),
        ))

        # HDR
        self.chk_hdr.toggled.connect(self.hdr_row.setEnabled)
        self.chk_hdr.toggled.connect(lambda _: self._on_any_control_changed())
        self.sld_hdr_amt.valueChanged.connect(lambda v: (
            self.lbl_hdr_amt.setText(f"{v/100:.2f}"),
            self._on_any_control_changed(),
        ))
        self.sld_hdr_knee.valueChanged.connect(lambda v: (
            self.lbl_hdr_knee.setText(f"{v/100:.2f}"),
            self._on_any_control_changed(),
        ))
        self.sld_hdr_knee.sliderPressed.connect(lambda: setattr(self, "_hdr_knee_user_locked", True))

        # Luma blend slider
        self.sld_luma_blend.valueChanged.connect(lambda v: (
            self.lbl_luma_blend.setText(f"{v/100:.2f}"),
            self._on_any_control_changed(),
        ))

        # Checkboxes that affect output
        self.chk_linked.toggled.connect(lambda _: self._on_any_control_changed())
        self.chk_normalize.toggled.connect(lambda _: self._on_any_control_changed())
        self.cmb_luma.currentTextChanged.connect(lambda _: self._on_any_control_changed())
        ...
        self._full_timer.setInterval(800)
        self._full_timer.timeout.connect(self._do_preview)

        # Clip stats debounce timer
        self._clip_timer = QTimer(self)
        self._clip_timer.setSingleShot(True)
        self._clip_timer.setInterval(500)
        self._clip_timer.timeout.connect(self._do_clip_stats)

        # Rebuild _schedule_clip_stats now that timer exists
        def _schedule_clip_stats():
            if getattr(self, "_job_running", False):
                return
            if sip.isdeleted(self):
        ...
        self.sld_target.valueChanged.connect(self._save_settings)
        self.chk_auto_refresh.toggled.connect(self._save_settings)
    ...
    def _on_any_control_changed(self):
        ...
        if not self.chk_auto_refresh.isChecked():
            return
        if getattr(self, "_job_running", False):
            return
        self._full_timer.start()
    ...
    def _suggest_hdr_knee_from_target(self):
    ...
    def _load_settings(self):
        ...
        target = float(s.value("stat_stretch/target_median", 0.25))
        self.spin_target.blockSignals(True)
        self.sld_target.blockSignals(True)
        self.spin_target.setValue(target)
        self.sld_target.setValue(int(round(target * 1000)))
        ...
        self.chk_auto_refresh.setChecked(bool(s.value("stat_stretch/auto_refresh", False, type=bool)))
    ...
    def _save_settings(self, *_):
        ...
        s.setValue("stat_stretch/auto_refresh", self.chk_auto_refresh.isChecked())
    ...
    def _reset_defaults(self):
        ...
        DEFAULT_LUMA_BLEND  = 60
        DEFAULT_CURVES_ON   = False
        DEFAULT_CURVES_STRENGTH = 20
        ...
        try:
            ...
            self.sld_target.setValue(int(round(DEFAULT_TARGET * 1000)))
    ...
    def _stretch_params(self) -> dict:
        """Collect current UI params into a dict for _run_stretch / _run_stretch_fast."""
        return dict(
            ...
            hdr_compress    = bool(self.chk_hdr.isChecked()),
            hdr_amount      = float(self.sld_hdr_amt.value()) / 100.0,
            hdr_knee        = float(self.sld_hdr_knee.value()) / 100.0,
            luma_only       = bool(self.chk_luma_only.isChecked()),
            luma_mode       = str(self.cmb_luma.currentText()),
            luma_blend      = float(self.sld_luma_blend.value()) / 100.0,
        )

    def _apply_stretch_to_array(self, imgf: np.ndarray, p: dict) -> np.ndarray | None:
        """Run the stretch functions on imgf with params dict p."""
        if imgf is None:
            return None
        is_mono = (imgf.ndim == 2 or (imgf.ndim == 3 and imgf.shape[2] == 1))
        if is_mono:
            out = stretch_mono_image(
                imgf.squeeze(),
                target_median    = p["target_median"],
                normalize        = p["normalize"],
                apply_curves     = p["apply_curves"],
                curves_boost     = p["curves_boost"],
                blackpoint_sigma = p["blackpoint_sigma"],
                no_black_clip    = p["no_black_clip"],
                hdr_compress     = p["hdr_compress"],
                hdr_amount       = p["hdr_amount"],
                hdr_knee         = p["hdr_knee"],
            )
        else:
            out = stretch_color_image(
                ...
                hdr_amount       = p["hdr_amount"],
                hdr_knee         = p["hdr_knee"],
                luma_only        = p["luma_only"],
                luma_mode        = p["luma_mode"],
                luma_blend       = p["luma_blend"],
            )
        return out
    ...
    def _run_stretch(self) -> np.ndarray | None:
        ...
        if imgf is None:
            return None
        p = self._stretch_params()
        out = self._apply_stretch_to_array(imgf, p)
    ...
    def _run_stretch_fast(self) -> np.ndarray | None:
        """Stretch on a downsampled image for quick auto-refresh preview."""
        imgf = self._get_source_float_fast()
        if imgf is None:
            return None
        p = self._stretch_params()
        out = self._apply_stretch_to_array(imgf, p)
        # No mask blending on fast preview (mask would need resizing too — skip for speed)
        return out
    ...
    def _do_clip_stats(self):
        ...
        if imgf.ndim == 2 or (imgf.ndim == 3 and imgf.shape[2] == 1):
            ...
            if luma_only or linked:
                ...
                if no_black_clip:
                    bp3 = np.array(
                        [float(rgb[..., 0].min()), float(rgb[..., 1].min()), float(rgb[..., 2].min())],
                        dtype=np.float32
                    )
                else:
        ...
        if (imgf.ndim == 3 and imgf.shape[2] == 3) and not (luma_only or linked):
            bp_disp = self.tr(f"R={float(bp3[0]):.6f}, G={float(bp3[1]):.6f}, B={float(bp3[2]):.6f}")
        else:
    ...
    def _start_stretch_job(self, mode: str):
        ...
        self._show_busy(
            "Statistical Stretch",
            "Processing preview…" if mode == "preview" else "Applying stretch…"
        )
    ...
    def _apply_out_to_doc(self, out: np.ndarray):
        ...
        p = self._stretch_params()
        target       = p["target_median"]
        linked       = p["linked"]
        normalize    = p["normalize"]
        apply_curves = p["apply_curves"]
        curves_boost = p["curves_boost"]
        blackpoint_sigma = p["blackpoint_sigma"]
        hdr_on       = p["hdr_compress"]
        hdr_amount   = p["hdr_amount"]
        hdr_knee     = p["hdr_knee"]
        luma_only    = p["luma_only"]
        luma_mode    = p["luma_mode"]
        ...
        if hasattr(mw, "mdi") and mw.mdi.activeSubWindow():
            ...
            if hasattr(mw, "_log"):
                mw._log(
                    ...
                    f"curves={'ON' if apply_curves else 'OFF'}"
                    f"{', boost='+str(round(curves_boost,2)) if apply_curves else ''}, "
        ...
        if obj is self.preview_scroll.viewport():
            if ev.type() == QEvent.Type.MouseButtonPress:
                ...
                self.preview_scroll.horizontalScrollBar().setValue(
                    self.preview_scroll.horizontalScrollBar().value() - delta.x()
                )
                self.preview_scroll.verticalScrollBar().setValue(
                    self.preview_scroll.verticalScrollBar().value() - delta.y()
                )
                return True
