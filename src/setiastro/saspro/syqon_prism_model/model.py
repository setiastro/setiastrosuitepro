...
# ============================================================================
# Lazy builders — torch imported only when a model is actually constructed
# ============================================================================

def _build_shared_blocks():
    """Returns (LayerNorm2d, SimpleGate) classes backed by torch."""
    import torch
    ...
    return LayerNorm2d, SimpleGate


def _build_FreeNAFNet():
    import torch
    import torch.nn as nn
    LayerNorm2d, SimpleGate = _build_shared_blocks()
    ...
    class FreeNAFBlock(nn.Module):
        def __init__(self, channels: int):
            ...
            self.dwconv = nn.Conv2d(
                channels * 2, channels * 2, kernel_size=3,
    ...
    class FreeNAFNet(nn.Module):
        ...
        def forward(self, x):
            ...
            for up, decoder in zip(self.ups, self.decoders):
                x = up(x); x = x + skips.pop(); x = decoder(x)
            x = self.ending(x)
...
def _build_DeepNAFNet():
    ...
    class DeepNAFBlock(nn.Module):
        ...
        def forward(self, x):
            y = self.norm1(x); y = self.conv1(y); y = self.dwconv(y)
            y = self.sg(y); y = self.ca(y); y = self.conv2(y)
            x = x + y * self.beta
            y = self.norm2(x); y = self.ffn1(y); y = self.sg(y); y = self.ffn2(y)
            x = x + y * self.gamma
            return x
...
# ============================================================================
# Backward-compatible alias (some code may import NAFNet directly)
# ============================================================================

# NAFNet is an alias for FreeNAFNet — constructed lazily on first use
class _NAFNetProxy:
    """
    Proxy that behaves like the FreeNAFNet class but only imports torch
    when actually instantiated.
    """
    def __call__(self, *args, **kwargs):
        return _build_FreeNAFNet()(*args, **kwargs)
