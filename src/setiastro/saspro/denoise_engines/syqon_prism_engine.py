# src/setiastro/saspro/denoise_engines/syqon_prism_engine.py
...
def _infer_nafnet_cfg_from_sd(sd: dict) -> Tuple[int, int]:
    base_ch = int(sd["intro.weight"].shape[0]) if "intro.weight" in sd else 48
...
def prism_denoise_rgb01(
    ...
    *,
    ...
    model, device, info, torch = load_prism_model(
        ckpt_path,
        use_gpu=use_gpu,
        prefer_dml=prefer_dml,
    )
    ...
    with torch.no_grad():
        for y0 in ys:
            for x0 in xs:
                ...
                den_patch = np.clip(pred, 0.0, 1.0).astype(np.float32, copy=False)
                den_patch = den_patch[:ph, :pw, :]
                wlocal = w2[:ph, :pw, :]

                out_acc[y0:y1, x0:x1, :] += den_patch * wlocal
                w_acc[y0:y1, x0:x1, :] += wlocal

                if callable(tile_cb):
                    tile_cb(y0, x0, ph, pw, den_patch)

                done += 1
                if callable(progress_cb):
                    progress_cb(done, total, "SyQon Prism tiles…")

    denoised = out_acc / np.maximum(w_acc, 1e-8)
    denoised = np.clip(denoised, 0.0, 1.0).astype(np.float32, copy=False)
