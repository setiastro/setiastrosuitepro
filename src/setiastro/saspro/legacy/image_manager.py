...
# =====================================================================
# Embedded metadata (sidecar) for non-FITS/XISF formats
# ---------------------------------------------------------------------
# FITS and XISF preserve the full header/keyword set natively. For TIFF,
# PNG, JPEG and WebP we serialize the same fits.Header we already carry
# internally (image_manager's ``original_header``) into that container's
# text-metadata slot, so OBJECT/EXPTIME/FILTER/DATE-OBS/WCS/HISTORY survive
# an export and are read back on the next load. Structural cards
# (SIMPLE/BITPIX/NAXISn/BSCALE/BZERO/...) are stripped on export because they
# describe the FITS *container*, not the science data, and are regenerated by
# each writer. Toggle with export_opts["embed_metadata"] (default True).
# =====================================================================
_SASPRO_META_MAGIC   = "saspro-metadata"
_SASPRO_META_VERSION = 1
_SASPRO_META_KEY     = "SASpro"   # PNG tEXt keyword / generic marker name

_FITS_STRUCTURAL_KEYS = {
    "SIMPLE", "EXTEND", "XTENSION", "PCOUNT", "GCOUNT", "BITPIX",
    "NAXIS", "NAXIS1", "NAXIS2", "NAXIS3", "NAXIS4",
    "BSCALE", "BZERO", "END",
}


def _header_for_sidecar(original_header) -> Optional[fits.Header]:
    """Copy of the header with container-structural cards removed, or None."""
    if isinstance(original_header, fits.Header):
        hdr = _drop_invalid_cards(original_header).copy()
    elif isinstance(original_header, dict):
        hdr = fits.Header()
    ...
    for k in list(hdr.keys()):
        if isinstance(k, str) and k.upper() in _FITS_STRUCTURAL_KEYS:
            try:
                del hdr[k]
            except Exception:
                pass
...
def encode_metadata_sidecar(original_header) -> Optional[str]:
    """Serialize a fits.Header into a compact JSON envelope for embedding."""
    hdr = _header_for_sidecar(original_header)
    if hdr is None:
        return None
    try:
        payload = {
            "format":      _SASPRO_META_MAGIC,
            "version":     _SASPRO_META_VERSION,
            "fits_header": hdr.tostring(sep="\n", endcard=False, padding=False),
        }
        return json.dumps(payload, ensure_ascii=False)
    except Exception as e:
        print(f"[metadata] Failed to encode embedded metadata: {e}")
        return None


def decode_metadata_sidecar(text) -> Optional[fits.Header]:
    """Inverse of encode_metadata_sidecar; tolerant of junk / foreign metadata."""
    if not text:
        return None
    if isinstance(text, (bytes, bytearray)):
        try:
            ...
            return None
    text = text.strip()
    if not text or _SASPRO_META_MAGIC not in text:
        return None
    try:
        payload = json.loads(text)
    except Exception:
        return None
    if not isinstance(payload, dict) or payload.get("format") != _SASPRO_META_MAGIC:
        return None
    raw = payload.get("fits_header")
    if not isinstance(raw, str) or not raw.strip():
        return None
    try:
        return _drop_invalid_cards(fits.Header.fromstring(raw, sep="\n"))
    except Exception as e:
        print(f"[metadata] Failed to decode embedded metadata: {e}")
        return None


def read_embedded_metadata_header(filename) -> Optional[fits.Header]:
    """Extract an embedded SASpro header from a TIFF/PNG/JPEG/WebP file, if present."""
    ext = filename.lower()
    text = None
    try:
        if ext.endswith((".tif", ".tiff")):
            with tiff.TiffFile(filename) as tf:
                text = getattr(tf.pages[0], "description", None) or None
        elif ext.endswith(".png"):
            with Image.open(filename) as im:
                info = {}
                info.update(getattr(im, "text", {}) or {})
                info.update(im.info or {})
                text = info.get(_SASPRO_META_KEY)
        elif ext.endswith((".jpg", ".jpeg")):
            with Image.open(filename) as im:
                text = (im.info or {}).get("comment")
        elif ext.endswith(".webp"):
            with Image.open(filename) as im:
                info = im.info or {}
                text = info.get("xmp") or info.get("comment")
    except Exception as e:
        print(f"[metadata] Could not read embedded metadata from {filename}: {e}")
        return None
    hdr = decode_metadata_sidecar(text)
    if hdr is not None:
        print(f"[metadata] Recovered {len(hdr)} embedded header cards from {os.path.basename(filename)}")
    return hdr
    ...
    while attempt <= max_retries:
        try:
            ...
            if filename.lower().endswith(('.fits', '.fit', '.fts', '.fits.gz', '.fit.gz', '.fz')):
                ...
                # Recover SASpro metadata embedded in ImageDescription (if any)
                if original_header is None:
                    original_header = read_embedded_metadata_header(filename)
            ...
            if return_metadata:
                ...
                _tail_meta = attach_wcs_to_metadata(_tail_meta, original_header)
                return image, original_header, bit_depth, is_mono, _tail_meta
            return image, original_header, bit_depth, is_mono
...
def save_image(img_array,
    ...
    try:
        ...
        if fmt == "png":
            ...
            if export_opts.get("embed_metadata", True):
                _meta_text = encode_metadata_sidecar(original_header)
                if _meta_text:
                    from PIL import PngImagePlugin as _PngPlugin
                    _pnginfo = _png_kwargs.get("pnginfo") or _PngPlugin.PngInfo()
                    _pnginfo.add_text(_SASPRO_META_KEY, _meta_text, zip=True)
                    _png_kwargs["pnginfo"] = _pnginfo
        ...
        if fmt == "jpg":
            ...
            if export_opts.get("embed_metadata", True):
                _meta_text = encode_metadata_sidecar(original_header)
                if _meta_text:
                    _mb = _meta_text.encode("utf-8")
                    if len(_mb) <= 65500:   # JPEG COM marker caps at ~64 KB
                        _jpg_kwargs["comment"] = _mb
                    else:
                        print(f"[metadata] JPEG comment too large ({len(_mb)} bytes); skipping.")
            try:
            ...
            if export_opts.get("embed_metadata", True):
                ...
                if _meta_text:
                    try:
                        ...
                        _params = _inspect.signature(save_webp).parameters
                        if "xmp" in _params:
                            _webp_meta_kwargs["xmp"] = _meta_text.encode("utf-8")
                        elif "metadata" in _params:
                            _webp_meta_kwargs["metadata"] = _meta_text
                        elif "exif" in _params:
                            _webp_meta_kwargs["exif"] = _meta_text.encode("utf-8")
                        else:
                            print("[metadata] save_webp accepts no metadata/xmp/exif "
                                  "param; skipping embedded metadata for WebP.")
                    except Exception as _mw:
                        print(f"[metadata] WebP metadata probe failed: {_mw}")
            save_webp(
                filename,
                img_array,
                lossless=lossless,
        ...
        if fmt in ("tif",):
            ...
            # --- SASpro metadata (written to TIFF ImageDescription) ---
            if export_opts.get("embed_metadata", True):
                _meta_text = encode_metadata_sidecar(original_header)
                if _meta_text:
                    kwargs["description"] = _meta_text
                    kwargs["metadata"] = None   # keep tifffile from overwriting ImageDescription
            if bd == "8-bit":
