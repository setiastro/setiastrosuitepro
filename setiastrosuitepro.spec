...
a = Analysis(
    ...
    hookspath=[str(PROJECT_ROOT)],  # keep if you have local hooks
    hooksconfig={},
    runtime_hooks=[
        str(PROJECT_ROOT / 'minimize_console.py') if (PROJECT_ROOT / 'minimize_console.py').exists() else '',
        str(PROJECT_ROOT / 'disable_zfpy.py')     if (PROJECT_ROOT / 'disable_zfpy.py').exists() else '',
    ],
...
# drop empty runtime hooks (when files are absent)
a.runtime_hooks = [rk for rk in a.runtime_hooks if rk]
...
exe = EXE(
    ...
    strip=False,
    upx=True,                 # set False if UPX causes issues on your box
    upx_exclude=[],
    runtime_tmpdir=None,
    console=True,             # set False if you want a windowed app only
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    icon=str(PROJECT_ROOT / 'astrosuitepro.ico') if (PROJECT_ROOT / 'astrosuitepro.ico').exists() else None,
)
